...
####### Histrogram
m_sugar  <- with(food, weighted.mean(sugar, na.rm = TRUE))
sd_sugar <- with(food, {mu <- mean(sugar, na.rm = TRUE)
  sqrt(mean((sugar - mu)^2, na.rm = TRUE))})

g2 <- ggplot(food, aes(x = sugar)) +
  geom_histogram(
...
ggsave("output/sugar_dist.png", plot = g2, width = 8, height = 6, dpi = 300)
